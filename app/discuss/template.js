...
function(T) {
	...
	T({
		...
		"content" : '<li class="hijackdata" data-nick="{{unick}}" data-xpt="{{xpt}}" data-replytodiscussid="{{id}}"><div class="feed-pic share-pic"><a href="{{ulink}}" target="_blank"><img data-card-action="xpt={{xpt}}" data-card="true" alt="" src="{{uavatar}}"></a></div><div class="share-con"><div class="mod-head">{{+username}}<span class="user-suffix"> </span>{{{content}}}</div><div class="mod-foot"><span class="feed-set"> {{#showDelete}}<a href="javascript:;" action="discuss.delete">删除</a>{{/showDelete}} <a action="app::discuss::discuss.replyto" href="javascript:;">回复</a></span><span class="feed-timestamp">{{time_ago}}</span></div></div></li>'
	}));
});
