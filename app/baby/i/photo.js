...
var project = {
	...
	ajax: {
		'cur': 'new',//当前接口地址
		'new': '/a/app/baby/newBabyPic.ac',//最新
		'hot': '/a/app/baby/babyHotPic.ac',//最热
		'friend': '/a/app/baby/myFriendsPic.ac',//好友
		'my': '/a/app/baby/myBabyPic.ac'//我的
	},
	emptyText: {
		'new': '',//最新
		'hot': '',//最热
		'friend': '生活很精彩所以很忙，好友们没有发照片日历',//好友
		'my': '生活很精彩所以很忙，TA还没有发照片日历'//我的
	},
	init: function($this,options){
		...
		*/
		...
		.delegate('div.baby-photo-nav-wrapper li','click.'+app,function(){
			var $li = $(this),
				$a = $li.find('> a');

			if($li.hasClass('curr')) return;
	...
	loadPhoto: function(pageNum){
		...
		$.getJSON(this.ajax[this.ajax.cur],{page: pageNum},function(results){
			if(results.code == 0){
				self._build(results.data,self.ajax.cur);
			}else{
				$.alert(results.msg);
			}
	...
	_build: function(data,cur){
		var $box = this.$this.find('div.baby-photo-list-wrapper'),
			obj,
			nowTime = new Date().getTime(),
			html = ['<ul class="clearfix">'];
		if(data.feed.length == 0){
			$box.html('<div class="baby-con-tips-wrapper">'+this.emptyText[cur]+'</div>');
			return;
		}
