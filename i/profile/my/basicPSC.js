		...
		checkInput: function() {
			...
			esname.bind('focus', function(e) {
				...
				*/
				...
				// if(false) {
				// 	etp.html('<span class="profile-input-error-clew"><em>昵称已被占用</em></span>');
				// 	return false;
				// }
				if(elov == elv) {
					$(this).removeClass('profile-input-error');
				...
				var _self = this;
				etp.html('<span class="profile-icon"><span class="profile-icon-loading"></span></span>');
				//http://i.sohu.com/a/profile/service/checksname.htm?sname=sohu
				var url = '/a/profile/service/checksname.htm?sname=' + encodeURIComponent(elv) + '&vn=snamesright&_input_encode=UTF-8';
				$.getScript(url, function() {
					if(window['snamesright']['code'] == 0) {
						etp.html('<i class="global-icon-right-12">正确</i>');
						$(_self).removeClass('profile-input-error');
					} else {
						etp.html('<span class="profile-input-error-clew"><em><i class="global-icon-error-12">错误</i>帐号已被占用</em></span>');
						$(_self).addClass('profile-input-error');
					}
				});
				// etp.html('<span class="profile-icon"><span class="profile-icon-right"></span></span>');
				return false;
			});
