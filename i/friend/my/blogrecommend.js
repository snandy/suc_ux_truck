...
var BlogRecommendDlg = function(opts){
	...
	function initGroups(){
		$.getJSON('http://i.sohu.com/a/app/friend/group/getgroups.do',function(results){
			if(results.code == 1){
	...
	this.show = function(data){
		...
		selected = [];
		$container.find('div.f-add-commend-search input[name="nick"]').val('');
...
var project = {
	...
	del: function($o){
		...
		$[app].removeRecommend(xpt,function(){
			$o.remove();
