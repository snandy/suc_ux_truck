	...
	Selection.prototype = {
		...
		getText : function(){
			...
			// if (this.isFocus()) {
				...
				// return nativeRange.text !== undefined ? nativeRange.text : '';
				if (browser.ie) {
					txt = this.getIERange().text
				} else {
					var frag = this.getRange().cloneContents();
					if (frag) {
						txt = domUtils.getInnerText(frag);
					}
				}
			// }
			return txt;
...
Editor.prototype = /**@lends baidu.editor.Editor.prototype*/{
	...
	_setup: function ( doc ) {
		...
		if (this.options.minFrameHeight) {
			...
			this.body.style.height = this.options.minFrameHeight-20+'px';
		}
	...
	setContent: function ( html,notFireSelectionchange) {
		var me = this, lastTagName, inline = utils.extend({a:1,A:1},dtd.$inline,true);
			
		// 过滤掉iframe tag
		// var re_iframe = /(<iframe){1,}[^<>]*>[^\0]*(<\/iframe>){1,}/g;
		html = html
			// .replace(re_iframe, '')
			.replace(/^[ \t\r\n]*?</,'<')
			.replace(/>[ \t\r\n]*?$/,'>')
	...
	execCommand : function ( cmdName ) {
		...
		if ( !cmd.notNeedUndo && !me.__hasEnterExecCommand ) {
			...
			if(me.queryCommandState(cmdName) !=-1){
				...
				// 解决ff下失效
				if (browser.gecko && cmdName=='source') {
					setTimeout(function() {
						result = me._callCmdFn( 'execCommand', args );
					}, 10);
				} else {
					result = me._callCmdFn( 'execCommand', args );
				}
...
UE.commands['insertimage'] = {
	execCommand : function (cmd, opt){
		...
		function joinImg(ci) {
			var str = '<img src="'+ci.src+'" '+ (ci.data_ue_src ? ' data_ue_src="' + ci.data_ue_src +'" ':'') +
					(ci.width ? 'width="'+ci.width+'" ':'') +
					(ci.height ? ' height="'+ci.height+'" ':'') +
					(ci['floatStyle']&&ci['floatStyle']!='center' ? ' style="float:'+ci['floatStyle']+';"':'') +
					(ci.title ? ' title="'+ utils.delhtml(ci.title) +'"':'') + ' border="'+ (ci.border||0) + '" hspace = "'+
			...
			if(ci.title){
				if(ci['floatStyle'] == 'center') {
					str += ci.title ? ('<p style="text-align: center">' + ci.title + '</p>') : '';
				}else{
					str += ci.title ? ('<p>' + ci.title + '</p>') : '';
				}
			}
	...
	UE.plugins['autofloat'] = function() {
		...
		function setFloating(){
			...
			if (LteIE6 || quirks) {
				...
				toolbarBox.style.top = (document.body.scrollTop||document.documentElement.scrollTop) - orgTop + 30+ 'px';
			} else {
				if (browser.ie7Compat && flag) {
				   flag = false;
				   toolbarBox.style.left =  getPosition(toolbarBox).left - document.documentElement.getBoundingClientRect().left+0  + 'px';
				}
				if(toolbarBox.style.position != 'fixed'){
					toolbarBox.style.position = 'fixed';
					toolbarBox.style.top = '30px';
...
UE.plugins['serialize'] = function () {
	...
	var me = this, EMPTY_TAG = dtd.$empty;
	...
	var	parseHTML = function () {
			...
			return function ( html, forceDtd ) {
				...
				parse( html, {
					...
					onTagClose: function ( tag ) {
						...
						if (node) {
							...
							// 如果没有找到开始标签, 则创建新标签 eg. </div> => <div></div>
				...
				// 处理这种情况, 只有开始标签没有结束标签的情况, 需要关闭开始标签 eg. <table>
	...
	var toHTML = function () {
		function printChildren( node, pasteplain ) {
			var children = node.children, buff = [];
...
function transNode( node, word_img_flag ) {
	...
	var sizeMap = [0, 10, 12, 16, 18, 24, 32, 48],
			attr,
			url = 'http://s3.suc.itc.cn/mysohu/ueditor/';
	...
	switch ( node.tag ) {
		case 'img':
			...
			if ( node.attributes.src && /^(?:file)/.test( node.attributes.src ) ) {
				...
				node.attributes.src = url + 'themes/default/images/spacer.gif';
				var flag = parseInt(node.attributes.width)<128||parseInt(node.attributes.height)<43;
				node.attributes.style="background:url(" + url +"themes/default/images/"+(flag?"word.gif":"localimage.png")+") no-repeat center center;border:1px solid #ddd";
...
(function (){
	...
	Separator.prototype = {
		...
		getHtmlTpl: function (){
			return '<div class="separtorLine" style="float:left;"><div id="##" class="edui-box %%"></div></div>';
		}
	};
...
(function (){
	...
	Button.prototype = {
		...
		//dodolook13399 在这里绘制 toolbar
		getHtmlTpl: function (){
		
		var TempHTMLString = '';
		switch(this.className){
			//edui-for-savekeke 保存按钮，自动保存
			case 'edui-for-save':
				TempHTMLString = '<div style="float:left;width:36px;height:50px;" class="IconGroupsWrapBox"><div class="IconWrapBox" style="float:left;width:36px;height:50px;"> <div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_save" class="edui-box edui-icon edui-for-save-icon"></div>' : '') +
			...
			//撤销和重做
			case 'edui-for-undo':
				TempHTMLString = '<div style="float:left;width:51px;height:50px;" class="IconGroupsWrapBox"> <div class="IconWrapBox" style="float:left;width:51px;height:23px;"> <div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_undo" class="edui-box edui-icon edui-for-undo-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div>';
			...
			case 'edui-for-redo':
				TempHTMLString = '<div class="IconWrapBox" style="float:left;margin-top:4px;width:51px;height:23px;"><div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div  data-logid="editor_redo" class="edui-box edui-icon edui-for-redo-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div> </div>';
			...
			case 'edui-for-paste':
				TempHTMLString =  '<div style="float:left;width:36px;height:50px;" class="IconGroupsWrapBox"><div class="IconWrapBox" style="float:left;width:36px;height:50px;"><div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_paste" class="edui-box edui-icon "></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div></div>';
			...
			//剪切
			case 'edui-for-cut':
				TempHTMLString =  '<div class="IconGroupsWrapBox" style="float:left;width:23px;height:50px;"><div class="IconWrapBox" style="float:left;width:23px;height:23px;"><div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_cut" class="edui-box edui-icon "></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div> ';
			...
			//复制
			case 'edui-for-copy':
				TempHTMLString = '<div class="IconWrapBox" style="float:left;width:23px;height:23px;"> <div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_copy" class="edui-box edui-icon "></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div></div>';
			...
			//加粗 edui-for-bold 斜体 下划线 字体颜色  背景颜色  删除线 
			case 'edui-for-bold':
				TempHTMLString =  '<div class="IconListLine" style="width:180px;clear:both;">'+
					' <div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_bold" class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div></div></div>';
			break;
			...
			//斜线 edui-for-italic
			case 'edui-for-italic':
				TempHTMLString =  '<div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_italic" class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div></div></div>';
			break;
			
			//下划线 edui-for-underline
			case 'edui-for-underline':
				TempHTMLString =  '<div data="line2_3" id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_underline" class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div></div></div>';
			break;
			
			//删除线 edui-for-strikethrough
			case 'edui-for-strikethrough':
				TempHTMLString =  '<div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_strikethrough" class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div></div></div>';
			break;

			//edui-for-removeformat || edui-for-autotypeset  
			//清除格式和一键排版
			case 'edui-for-removeformat':
				TempHTMLString = '<div style="float:left;width:77px;height:50px;" class="IconGroupsWrapBox"><div class="IconWrapBox" style="float:left;width:77px;height:23px;"><div style="width:77px;" id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_removeformat" class="edui-box edui-icon edui-for-removeformat-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div>';
			...
			//纯文本粘贴功能 edui-for-pasteplain
			case 'edui-for-pasteplain':
				TempHTMLString = '<div id="##" class="edui-box %%">' +
										'<div id="##_state" stateful>' +
										 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
										 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
										  (this.showIcon ? '<div data-logid="editor_pasteastxt" class="edui-box edui-icon"></div>' : '') +
										  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
										 '</div>' +
									'</div></div>'+
								'</div>';
			...
			//首行缩进 edui-for-indent
			case 'edui-for-indent':
				TempHTMLString =  '<div id="##" class="edui-box %%">' +
										'<div id="##_state" stateful>' +
										 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
										 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
										  (this.showIcon ? '<div data-logid="editor_indent"  class="edui-box edui-icon"></div>' : '') +
										  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
										 '</div>' +
										'</div>' +
										'</div></div>'+
			...
			//第二行的 第一个 居左对齐 edui-for-justifyleft
			case 'edui-for-justifyleft':
				TempHTMLString =  '<div class="IconListLine" >'+
										'<div id="##" class="edui-box %%">' +
											'<div id="##_state" stateful>' +
											 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
											 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
											  (this.showIcon ? '<div data-logid="editor_alignleft"  class="edui-box edui-icon"></div>' : '') +
											  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
											 '</div>' +
										'</div></div>'+
									'</div>';
			...
			//居中对齐
			case 'edui-for-justifycenter':
				TempHTMLString =  '<div id="##" class="edui-box %%">' +
										'<div id="##_state" stateful>' +
										 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
										 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
										  (this.showIcon ? '<div data-logid="editor_aligncenter" class="edui-box edui-icon"></div>' : '') +
										  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
										 '</div>' +
									'</div></div>'+
								'</div>';
			...
			//居右对齐
			case 'edui-for-justifyright':
				TempHTMLString =  '<div id="##" class="edui-box %%">' +
										'<div id="##_state" stateful>' +
										 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
										 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
										  (this.showIcon ? '<div data-logid="editor_alignright" class="edui-box edui-icon"></div>' : '') +
										  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
										 '</div>' +
									'</div></div>'+
								'</div>';
			...
			//全选
			case 'edui-for-selectall':
				TempHTMLString =  '<div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_seletall" class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div> ';
			...
			//edui-for-link  【链接  表情 】|图片|视频
			case 'edui-for-link':
				TempHTMLString ='<div class="IconGroupsWrapBox" style="float:left;height:50px;width:25px;">'+		
							'<div id="##" class="edui-box %%">' +
								'<div id="##_state" stateful>' +
								 '<div class="%%-wrap" style="width:23px; height:23px; overflow:hidden;"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
								 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
								  (this.showIcon ? '<div data-logid="editor_link" class="edui-box edui-icon"></div>' : '') +
								  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
								 '</div>' +
							'</div></div>'+
						'</div>';
			...
			case 'edui-for-insertimage':
				TempHTMLString =  '<div style="float:left; "><div data-name="ImageIconWrap" style="width:36px;height:50px;display:block;" id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_image" class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div>';
			...
			case 'edui-for-insertvideo':
				TempHTMLString =  '<div style="float:left; "><div data-name="VideoIconWrap" style="width:36px;height:50px;display:block;" id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return false;">' +
					  (this.showIcon ? '<div data-logid="editor_video"  class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div>';
			...
			case 'edui-for-insertad': 
				TempHTMLString =  '<div style="float:left; "><div data-name="ADIconWrap" style="width:36px;height:50px;display:block;" id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_ad"  class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div>';
			...
			//HTML源代码-和-预览  edui-for-source edui-for-preview
			case 'edui-for-source':
				TempHTMLString =  '<div style="float:left; width:54px; height:50px;" class="IconGroupsWrapBox"><div class="IconWrapBox" style=""><div id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_html" class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div>';
			...
			case 'edui-for-preview':
				TempHTMLString =  '<div class="IconWrapBox" style=" width:54px; height:23px;"><div  id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
					  (this.showIcon ? '<div data-logid="editor_preview" class="edui-box edui-icon"></div>' : '') +
					  (this.showText ? '<div class="edui-box edui-label">' + this.label + '</div>' : '') +
					 '</div>' +
					'</div>' +
					'</div></div></div></div>';
			...
			default:
				TempHTMLString =  '<div data="dodolook12880" id="##" class="edui-box %%">' +
					'<div id="##_state" stateful>' +
					 '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + this.title + '"' : '') +
					 ' class="%%-body" onmousedown="return false;" onclick="return $$._onClick();">' +
...
(function (){
	...
	SplitButton.prototype = {
		...
		//绘制带下拉的按钮 dodolook13480
		getHtmlTpl: function () {
			var TempHTMLString = '';
			switch(this.className){
			
				//edui-for-fontfamily 字体  字号 edui-for-fontsize
				case 'edui-for-fontfamily':
					TempHTMLString =  '<div style="float:left; width:180px; height:50px;" class="IconGroupsWrapBox">' +
										'<div class="IconListLine" style="width:180px; ">'+
										'<div id="##" class="edui-box %%">' +
											'<div '+ (this.title ? 'title="' + this.title + '"' : '') +' id="##_state" stateful><div class="%%-body">' +
											'<div data-logid="editor_font" id="##_button_body" class="edui-box edui-button-body" onclick="$$._onButtonClick(event, this);">' +
				...
				case 'edui-for-fontsize':
					TempHTMLString =  '<div id="##" class="edui-box %%">' +
											'<div '+ (this.title ? 'title="' + this.title + '"' : '') +' id="##_state" stateful><div class="%%-body">' +
											'<div data-logid="editor_size" id="##_button_body" class="edui-box edui-button-body" onclick="$$._onButtonClick(event, this);">' +
											'<div class="edui-box edui-icon"></div>' +
											'</div>' +
											'<div class="edui-box edui-splitborder"></div>' +
											'<div class="edui-box edui-arrow" onclick="$$._onArrowClick();"></div>' +
				...
				//字体颜色 = edui-for-forecolor  和  背景颜色 = edui-for-backcolor 
				case 'edui-for-forecolor':
					TempHTMLString =  ' <div data="line2_5" id="##" class="edui-box %%">' +
											'<div '+ (this.title ? 'title="' + this.title + '"' : '') +' id="##_state" stateful><div class="%%-body">' +
											'<div data-logid="editor_textcolor" id="##_button_body" class="edui-box edui-button-body" onclick="$$._onButtonClick(event, this);">' +
											'<div class="edui-box edui-icon "></div>' +
											'</div>' +
											'<div class="edui-box edui-splitborder"></div>' +
											'<div class="edui-box edui-arrow" onclick="$$._onArrowClick();"></div>' +
				...
				case 'edui-for-backcolor':
					TempHTMLString =  ' <div data="line2_6" id="##" class="edui-box %%">' +
											'<div '+ (this.title ? 'title="' + this.title + '"' : '') +' id="##_state" stateful><div class="%%-body">' +
											'<div data-logid="editor_bgcolor" id="##_button_body" class="edui-box edui-button-body" onclick="$$._onButtonClick(event, this);">' +
											'<div class="edui-box edui-icon "></div>' +
											'</div>' +
											'<div class="edui-box edui-splitborder"></div>' +
											'<div class="edui-box edui-arrow" onclick="$$._onArrowClick();"></div>' +
				...
				//另外的一个大包，有序列表，无序列表 ，
				//有序列表 
				case 'edui-for-insertorderedlist':
					TempHTMLString ='<div class="IconGroupsWrapBox"  style="width:100px; float:left; height:50px;">'+  
										'<div class="IconListLine"   style="height:23px;">'+
										'<div data="mmxx" id="##" class="edui-box %%">' +
											'<div '+ (this.title ? 'title="' + this.title + '"' : '') +' id="##_state" stateful><div class="%%-body">' +
											'<div data-logid="editor_numberedlist" id="##_button_body" class="edui-box edui-button-body" onclick="$$._onButtonClick(event, this);">' +
											'<div class="edui-box edui-icon"></div>' +
											'</div>' +
											//'<div class="edui-box edui-splitborder"></div>' +
											//'<div class="edui-box edui-arrow" onclick="$$._onArrowClick();"></div>' +
				...
				//无序列表 
				case 'edui-for-insertunorderedlist':
					TempHTMLString =  '<div data="mmxx" id="##" class="edui-box %%">' +
											'<div '+ (this.title ? 'title="' + this.title + '"' : '') +' id="##_state" stateful><div class="%%-body">' +
											'<div data-logid="editor_nonenumberedlist" id="##_button_body" class="edui-box edui-button-body" onclick="$$._onButtonClick(event, this);">' +
											'<div class="edui-box edui-icon"></div>' +
											'</div>' +
											//'<div class="edui-box edui-splitborder"></div>' +
											//'<div class="edui-box edui-arrow" onclick="$$._onArrowClick();"></div>' +
				...
				case 'edui-for-emotion':
					TempHTMLString = 	'<div data="mmxx" id="##" class="edui-box %%">' +
												'<div '+ (this.title ? 'title="' + this.title + '"' : '') +' id="##_state" stateful><div class="%%-body">' +
												'<div data-logid="editor_smiley" id="##_button_body"  class="edui-box edui-button-body" onclick="$$._onButtonClick(event, this);">' +
												'<div class="edui-box edui-icon"></div>' +
												'</div>' +
												//'<div class="edui-box edui-splitborder"></div>' +
												//'<div class="edui-box edui-arrow" onclick="$$._onArrowClick();"></div>' +
				...
				//一键排版的功能按钮
				case 'edui-for-autotypeset':
					TempHTMLString =  '<div class="IconWrapBox" style="float:left; width:77px; height:23px; margin-top:2px; "> <div style="width:77px;" id="##" class="edui-box %%">' +
											'<div '+ (this.title ? 'title="' + this.title + '"' : '') +' id="##_state" stateful><div class="%%-body">' +
											'<div style="width:77px;" id="##_button_body" class="edui-box edui-button-body" onclick="$$._onButtonClick(event, this);">' +
											'<div  data-logid="editor_magic" style="width:77px; height:23px; display:block; " class="edui-box edui-icon  edui-for-autotypeset-icon"></div>' +
											'</div>' +
											//'<div class="edui-box edui-splitborder"></div>' +
											//'<div class="edui-box edui-arrow" onclick="$$._onArrowClick();"></div>' +
											'</div></div></div> </div> </div>';
...
(function (){
	...
	Toolbar.prototype = {
		...
		setdisabled: function(disabled) {
			...
			var exclude = ['保存', '源代码', '预览'];
			for (var i=0; i<items.length; i++) {
				var button = items[i];
				if (utils.inArray(button.title, exclude)) {
					continue;
				}
...
(function (){
	...
	var btnCmds = ['Undo', 'Redo','FormatMatch',
		...
		'DeleteCol','SplittoRows','SplittoCols','SplittoCells','MergeCells','DeleteTable','Save','Paste','Cut','Copy','Preview']
	...
	var dialogCmds = ['Anchor','Link', 'InsertImage', 'InsertVideo', 'InsertAD', 'wordimage']
	k = dialogCmds.length
	while (k--) {
		cmd = dialogCmds[k];
		editorui[cmd] = function (cmd){
			...
			return function (editor, iframeUrl, title){
				...
				iframeUrl = editor.ui.mapUrl(iframeUrl);
				title = title || editor.options.labelMap[cmd.toLowerCase()] || '';
				var isDraggable = cmd != 'insertimage';
				var dialog = new editorui.Dialog({
					iframeUrl: iframeUrl,
					autoReset: true,
			...
			items.push({
				...
				renderLabelHtml: function (){
					return '<div class="edui-label %%-label" style="line-height:1;font-size:' +
						this.value + '"><span style="font-size:12px; float:right;">' + this.value +'</span>'+ (this.label || '') + '</div>';
				}
			});
		}
		var ui = new editorui.Combox({
...
(function () {
	...
	EditorUI.prototype = {
		...
		initEditorUI: function () {
			...
			videoDialog.render();
			// 淘宝广告ad
			var adDialog = new baidu.editor.ui.Dialog( {
				iframeUrl: iframeUrlMap ? editor.ui.mapUrl( iframeUrlMap.insertad ) : '',
				autoReset: true,
				draggable: true,
				editor: editor,
				className: 'edui-for-insertad',
				title: '商品',
				buttons: [{
						className: 'edui-okbutton',
						label: '确认',
			...
			var popup = new baidu.editor.ui.Popup( {
				...
				_setIframeAlign: function( value ) {
					...
					switch ( value ) {
						...
						case 2:
							newFrame.setAttribute( "align", "middle" );break;
					}
			...
			function isWordImg(img) {
				return !!img.getAttribute('word_img');
			}
			editor.addListener( 'selectionchange', function ( t, causeByUi ) {
				...
				if ( img != null && img.tagName.toLowerCase() == 'img' ) {
					if ( img.getAttribute( 'anchorname' ) ) {
						...
						var btnName = isWordImg(img) ? '上传' : '修改';
						html += popup.formatHtml(
							...
							'<span onclick=$$._onImgSetFloat("left") class="edui-clickable">左环绕</span>&nbsp;' +
							'<span onclick=$$._onImgSetFloat("right") class="edui-clickable">右环绕</span>&nbsp;' +
							'<span onclick="$$._onImgEditButtonClick(event, this);" class="edui-clickable">' + btnName + '</span></nobr>' );
					}
				}
				var link;
		...
		renderToolbarBoxHtml: function () {
			var buff = [];
			buff.push( this.toolbar.renderHtml() );
			return buff.join( '' );
		},
	...
	function insertTitle() {
		var str = '<input id="blog-title" maxLength="50" style="color:#999;"/>';
		var el  = document.createElement('div');
			el.innerHTML = str;
		var toolbarEl = document.getElementById('edui1_toolbarbox');
		domUtils.insertAfter(toolbarEl, el);
		if (browser.ie && browser.version<8) {
