...
(function(window, undefined) {
	...
	jQuery.extend({
		...
		when : function(firstParam) {
			var args = arguments, i = 0, length = args.length, count = length, deferred = length <= 1 && firstParam
			&& jQuery.isFunction(firstParam.promise) ? firstParam : jQuery.Deferred();
			function resolveFunc(i) {
				return function(value) {
					args[i] = arguments.length > 1 ? sliceDeferred.call(arguments, 0) : value;
					if (!(--count)) {
						// Strange bug in FF4:
						// Values changed onto the arguments object sometimes
						// end up as undefined values
	...
	jQuery.extend({
		...
		data : function(elem, name, data, pvt /* Internal Use Only */) {
			...
			if (!cache[id]) {
				...
				// metadata on plain JS objects when the object is serialized
				// using
				// JSON.stringify
				if (!isNode) {
			...
			// TODO: This is a hack for 1.5 ONLY. It will be removed in 1.6.
			// Users should
		...
		removeData : function(elem, name, pvt /* Internal Use Only */) {
			...
			// We destroyed the entire user cache at once because it's faster
			// than
			...
			if (internalCache) {
				...
				// Otherwise, we need to eliminate the expando on the node to
				// avoid
	...
	function handleQueueMarkDefer(elem, type, src) {
		var deferDataKey = type + "defer", queueDataKey = type + "queue", markDataKey = type + "mark", defer = jQuery.data(
		elem, deferDataKey, undefined, true);
		if (defer && (src === "queue" || !jQuery.data(elem, queueDataKey, undefined, true))
	...
	.extend({
		...
		promise : function(type, object) {
			...
			type = type || "fx";
			var defer = jQuery.Deferred(), elements = this, i = elements.length, count = 1, deferDataKey = type + "defer", queueDataKey = type
			+ "queue", markDataKey = type + "mark", tmp;
			function resolve() {
				if (!(--count)) {
			...
			while (i--) {
				...
				|| (jQuery.data(elements[i], queueDataKey, undefined, true) || jQuery.data(elements[i], markDataKey, undefined,
				true)) && jQuery.data(elements[i], deferDataKey, jQuery._Deferred(), true))) {
					count++;
					tmp.done(resolve);
				}
			}
			...
			if (notxml) {
				...
				if (!hooks) {
					...
					if (rboolean.test(name)) {
						hooks = boolHook;
						// Use formHook for forms and if the name contains
						// certain characters
		...
		attrHooks : {
			...
			tabIndex : {
				get : function(elem) {
					...
					return attributeNode && attributeNode.specified ? parseInt(attributeNode.value, 10) : rfocusable
					.test(elem.nodeName)
	...
	jQuery.event = {
		...
		add : function(elem, types, handler, data) {
			...
			if (!eventHandle) {
				elemData.handle = eventHandle = function(e) {
					...
					return typeof jQuery !== "undefined" && (!e || jQuery.event.triggered !== e.type) ? jQuery.event.handle
		...
		trigger : function(event, data, elem, onlyHandlers) {
			...
			if (!elem) {
				...
				jQuery.each(jQuery.cache, function() {
					// internalKey variable is just used to make it easier to
					// find
		...
		handle : function(event) {
			...
			// events.
			var handlers = ((jQuery._data(this, "events") || {})[event.type] || []).slice(0), run_all = !event.exclusive
			&& !event.namespace, args = Array.prototype.slice.call(arguments, 0);
	...
	function(i, name) {
		jQuery.fn[name] = function(types, data, fn, origSelector /*
																	 ...
																	 * Use Only
																	 */) {
			var type, i = 0, match, namespaces, preType, selector = origSelector || this.selector, context = origSelector ? this
			: jQuery(this.context);
	...
	(function() {
		...
		var Sizzle = function(selector, context, results, seed) {
			...
			if (parts.length > 1 && origPOS.exec(selector)) {
				...
				if (!seed && parts.length > 1 && context.nodeType === 9 && !contextXML && Expr.match.ID.test(parts[0])
		...
		var Expr = Sizzle.selectors = {
			...
			relative : {
				"+" : function(checkSet, part) {
					var isPartStr = typeof part === "string", isTag = isPartStr && !rNonWord.test(part), isPartStrNotTag = isPartStr
					&& !isTag;
					if (isTag) {
					...
					for ( var i = 0, l = checkSet.length, elem; i < l; i++) {
						if ((elem = checkSet[i])) {
							...
							checkSet[i] = isPartStrNotTag || elem && elem.nodeName.toLowerCase() === part ? elem || false
							: elem === part;
						}
					}
					if (isPartStrNotTag) {
			...
			preFilter : {
				...
				CHILD : function(match) {
					if (match[1] === "nth") {
						...
						// '3n+2', '4n-1', '-n+6'
						var test = /(-?)(\d*)(?:n([+\-]?\d*))?/.exec(match[2] === "even" && "2n" || match[2] === "odd"
						&& "2n+1" || !/\D/.test(match[2]) && "0n+" + match[2] || match[2]);
			...
			filter : {
				...
				ATTR : function(elem, match) {
					var name = match[1], result = Expr.attrHandle[name] ? Expr.attrHandle[name](elem)
					: elem[name] != null ? elem[name] : elem.getAttribute(name), value = result + "", type = match[2], check = match[4];
					return result == null ? type === "!=" : type === "=" ? value === check : type === "*=" ? value
					.indexOf(check) >= 0 : type === "~=" ? (" " + value + " ").indexOf(check) >= 0 : !check ? value
					&& result !== false : type === "!=" ? value !== check : type === "^=" ? value.indexOf(check) === 0
					: type === "$=" ? value.substr(value.length - check.length) === check : type === "|=" ? value === check
				...
				Sizzle = function(query, context, extra, seed) {
					...
					if (!seed && !Sizzle.isXML(context)) {
						...
						if (context.nodeType === 9) {
							...
							var oldContext = context, old = context.getAttribute("id"), nid = old || id, hasParent = context.parentNode, relativeHierarchySelector = /^\s*[+~]/
							.test(query);
							if (!old) {
	...
	jQuery.fn.extend({
		...
		add : function(selector, context) {
			var set = typeof selector === "string" ? jQuery(selector, context) : jQuery
			.makeArray(selector && selector.nodeType ? [selector] : selector), all = jQuery.merge(this.get(), set);
	...
	jQuery.extend({
		filter : function(expr, elems, not) {
			...
			return elems.length === 1 ? jQuery.find.matchesSelector(elems[0], expr) ? [elems[0]] : [] : jQuery.find.matches(
			expr, elems);
		},
	...
	jQuery.fn.extend({
		...
		replaceWith : function(value) {
			if (this[0] && this[0].parentNode) {
				...
				return this.length ? this.pushStack(jQuery(jQuery.isFunction(value) ? value() : value), "replaceWith", value)
				: this;
			}
		},
			...
			for ( var i = 0, elem; (elem = elems[i]) != null; i++) {
				...
				if (typeof elem === "string") {
					if (!rhtml.test(elem)) {
						...
						// expected
						var tag = (rtagName.exec(elem) || ["", ""])[1].toLowerCase(), wrap = wrapMap[tag] || wrapMap._default, depth = wrap[0], div = context
						.createElement("div");
	...
	if (!jQuery.support.opacity) {
		jQuery.cssHooks.opacity = {
			get : function(elem, computed) {
				...
				return ropacity.test((computed && elem.currentStyle ? elem.currentStyle.filter : elem.style.filter) || "") ? (parseFloat(RegExp.$1) / 100)
				+ ""
				: computed ? "1" : "";
			},
			set : function(elem, value) {
				...
				var opacity = jQuery.isNaN(value) ? "" : "alpha(opacity=" + value * 100 + ")", filter = currentStyle
	...
	.extend({
		...
		ajax : function(url, options) {
			...
			globalEventContext = callbackContext !== s && (callbackContext.nodeType || callbackContext instanceof jQuery) ? jQuery(callbackContext)
			: jQuery.event,
			// Deferreds
	...
	jQuery.ajaxPrefilter("json jsonp", function(s, originalSettings, jqXHR) {
		...
		if (s.dataTypes[0] === "jsonp" || s.jsonp !== false && (jsre.test(s.url) || inspectData && jsre.test(s.data))) {
			var responseContainer, jsonpCallback = s.jsonpCallback = jQuery.isFunction(s.jsonpCallback) ? s.jsonpCallback()
			: s.jsonpCallback, previous = window[jsonpCallback], url = s.url, data = s.data, replace = "$1" + jsonpCallback
			+ "$2";
			if (s.jsonp !== false) {
	...
	// opacity animations
	["opacity"]], fxNow, requestAnimationFrame = window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame
	...
	if ("getBoundingClientRect" in document.documentElement) {
		jQuery.fn.offset = function(options) {
			...
			var body = doc.body, win = getWindow(doc), clientTop = docElem.clientTop || body.clientTop || 0, clientLeft = docElem.clientLeft
			|| body.clientLeft || 0, scrollTop = win.pageYOffset || jQuery.support.boxModel && docElem.scrollTop
			|| body.scrollTop, scrollLeft = win.pageXOffset || jQuery.support.boxModel && docElem.scrollLeft || body.scrollLeft, top = box.top
	...
	jQuery.each(["Left", "Top"], function(i, name) {
		...
		jQuery.fn[method] = function(val) {
			...
			if (val === undefined) {
				...
				&& win.document.documentElement[method] || win.document.body[method] : elem[method];
			}
	...
	jQuery.each(["Height", "Width"], function(i, name) {
		...
		jQuery.fn[type] = function(size) {
			...
			if (jQuery.isWindow(elem)) {
				...
				return elem.document.compatMode === "CSS1Compat" && docElemProp || elem.document.body["client" + name]
				|| docElemProp;
