...
// 2011-12-19 修改，点击事件打开原图 line{30~38}
(function($,mysohu) {
	$.fn.dragsort = function(options) {
		...
		this.each(function(i, cont) {
			...
			var newList = {
				...
				init: function() {
					//当鼠标按下和抬起的坐标相同时，打开原图链接.begin
					$(this.container).unbind("mousedown");
					$(this.container).unbind("mouseup");
					
					//$(this.container).attr("data-listIdx", i).mousedown(function(e){
					$(this.container).bind("mousedown",function(e){
						this.fixation = e.pageX + "_" + e.pageY;
					}).bind("mouseup",function(e){
						if(this.fixation == (e.pageX + "_" + e.pageY)){
							
							var targetAction = $(e.target).attr("data-action");
							var targetURL = $(e.target).parent().attr("href");
							var photoid = $(e.target).attr("data-photoid") || "";
							var photosetid = $(e.target).attr("data-albumid") || "";
							var userid = $(e.target).attr("data-userid") || "";
							var albumid = $("#albumID").val() || photosetid;
							var albumprivilege = $(e.target).attr("data-privilege") || "";
							
							var typeID = $('#photosetType').val() || 0;
							
							var param = {"AlbumID":albumid,"UserID":userid,"PhotoID":photoid,"AlbumPrivilege":albumprivilege,"TypeID":typeID};
							
							/*为了处理http://jira.nrcp.sohu.com/browse/SUC-4005
							如果action是editPhotoDesc则缓存数据到id=set_face_link的元素上*/
							
							if(targetAction =='editPhotoDesc'){
								$("#set_face_link").data("PhotoID",photoid);
							}
							
							if(targetAction != undefined){								
								mysohu.AlbumAction.init(targetAction,param);
							}else{
								...
								var CacheUndoneActionPhotoID = $("#set_face_link").data("PhotoID");
								if(CacheUndoneActionPhotoID != undefined){
									mysohu.AlbumAction.submitPhotoDescription(CacheUndoneActionPhotoID);
									$("#set_face_link").removeData("PhotoID");
								}
							}
						}
					});
					//当鼠标按下和抬起的坐标相同时，打开原图链接.end
