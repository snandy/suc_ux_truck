...
(function() {
	...
	function resolveDefs(c, block, def) {
		return block.toString().replace(c.define, function(match, code, assign, value) {
			...
			if (!(code in def)) {
				if (assign === ':') {
					...
					def[code] = window["eval"](value);
				}
			}
			return '';
		}).replace(c.use, function(match, code) {
			var v = window["eval"](code);
			return v ? resolveDefs(c, v, def) : v;
		});
	}
	doT.template = function(tmpl, c, def) {
		c = c || doT.templateSettings;
		var cstart = c.append ? "'+(" : "';out+=(", // optimal choice depends on
		// platform/size of
		// templates
		cend = c.append ? ")+'" : ");out+='";
		var str = (c.use || c.define) ? resolveDefs(c, tmpl, def || {}) : tmpl;
		str = ("var out='"
		+ ((c.strip) ? str.replace(/\s*<!\[CDATA\[\s*|\s*\]\]>\s*|[\r\n\t]|(\/\*[\s\S]*?\*\/)/g, '') : str).replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(c.interpolate,
		function(match, code) {
		...
		function(match, code) {
			return cstart + code.replace(/\\'/g, "'").replace(/\\\\/g, "\\").replace(/[\r\t\n]/g, ' ')
			+ ").toString().replace(/&(?!\\w+;)/g, '&#38;').split('<').join('&#60;').split('>').join('&#62;').split('" + '"' + "').join('&#34;').split(" + '"' + "'" + '"'
